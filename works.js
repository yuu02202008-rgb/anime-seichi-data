...
function render() {
  ...
  grid.innerHTML = visible.map((work, index) => {
    ...
    const artwork = official?.artworkDisabled ? null : (official?.artworkUrl ? { image:official.artworkUrl, credit:official.artworkCredit || "OFFICIAL" } : animeArtwork[work.name]);
